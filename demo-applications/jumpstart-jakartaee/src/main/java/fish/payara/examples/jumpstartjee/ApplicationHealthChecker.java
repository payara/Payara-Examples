		...
		return () -> {
			...
			return adequateMemory ?
					...
					HealthCheckResponse
							...
							.withData("app-status", "Jakarta EE app is NOT Live... Yet?")
							.withData("date", LocalDateTime.now(ZoneOffset.UTC)
									.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")))
							.down()
							.build();
		...
		return () -> {
			...
			return readinessCountdown.getCount() == 0 ?
					...
					HealthCheckResponse
							.named("jakarta-readiness")
							.down()
							.withData("app-status", "Jakarta EE app is NOT Ready!")
							.withData("date", LocalDateTime.now(ZoneOffset.UTC)
									.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")))
							.build();
		};
		...
		return () -> {
			...
			return startupCountdown.getCount() == 0 ?
					HealthCheckResponse
							...
							.up()
							.withData("app-status", "Jakarta EE app has started!")
							.withData("date", LocalDateTime.now(ZoneOffset.UTC)
									.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")))
							.build() :
					HealthCheckResponse
							...
							.down()
							.withData("app-status", "Jakarta EE app startup failed!!")
							.withData("date", LocalDateTime.now(ZoneOffset.UTC)
									.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")))
							.build();
		};
